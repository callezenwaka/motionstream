...
def generate_basic_report(scan_results, file_path=None):
    ...
    if file_path:
        report.append(bold(f"Security Analysis: {Path(file_path).name}"))
        report.append(f"\nScanned on {timestamp}\n")
    else:
        package_info = scan_results[0]['package'] if scan_results else "Unknown"
        report.append(bold(f"Security Analysis: {package_info}"))
        report.append(f"\nScanned on {timestamp}\n")
    ...
    if total_vulnerabilities > 0:
        if critical > 0:
            report.append("🚨 " + bold("CRITICAL SECURITY ISSUES FOUND"))
        elif high > 0:
            report.append("⚠️ " + bold("HIGH SECURITY ISSUES FOUND"))
        elif medium > 0:
            report.append("🔶 " + bold("MEDIUM SECURITY ISSUES FOUND"))
        else:
            report.append("ℹ️ " + bold("MINOR SECURITY ISSUES FOUND"))

        report.extend([
            f"* Found {bold(str(total_vulnerabilities))} vulnerabilities in {bold(str(len(vulnerable_packages)))} packages",
            "* Severity breakdown:",
            f"  - Critical: {bold(str(critical))}",
            f"  - High: {bold(str(high))}",
            f"  - Medium: {bold(str(medium))}",
            f"  - Low: {bold(str(low))}",
            f"  - Unknown: {bold(str(unknown))}" if unknown > 0 else ""
        ])
    else:
        report.append(bold("✅ No known vulnerabilities found"))
